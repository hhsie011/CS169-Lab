...
/*  if (backoffMode == 0) {
  ...
  else if (backoffMode == 1) {
    m_cw = std::min(m_cw + 1, m_cwMax);
  }
*/
  m_cw = std::min (m_cw + 17, m_cwMax);
}
...
DcfManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DcfManager")
    ...
    .AddAttribute ("BackoffMode",
                   "Exponential or linear backoff",
		   DoubleValue(0),
		   MakeDoubleAccessor (&DcfManager::m_backoffmode),		         
...
double 
DcfManager::GetBackoffMode ()
{
  return m_backoffmode;
}
*/
void
...
DcfManager::Add (DcfState *dcf)
{
  ...
  //dcf->backoffMode = (this->GetBackoffMode() < 1)? 0 : 1; 
  ...
  if (accessTimeoutNeeded)
    {
      ...
      totalDelay += expectedBackoffDelay;
